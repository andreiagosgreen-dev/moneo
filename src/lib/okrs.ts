 ...
 */
export function okrReview(objectives: Objective[], period?: string, i18n: I18n = EN_I18N): string {
  const { t } = i18n;
  const roots = rootObjectives(objectives, period);
  const label = period ?? t('okr.review.allPeriods');
  if (roots.length === 0) return t('okr.review.empty', { label });
  const lines = [
    t('okr.review.header', { label, pct: String(overallOkrProgress(objectives, period)) }),
  ];
  for (const o of roots) {
    const pct = objectiveProgress(objectives, o.id);
    lines.push(t('okr.review.objectiveLine', { title: o.title, pct: String(pct) }));
    ...
    if (ranked.length > 0) {
      ...
      lines.push(
        t('okr.review.bestKr', {
          title: best.title,
          pct: String(Math.round(krProgress(best) * 100)),
        }),
      );
      if (worst.id !== best.id) {
        lines.push(
          t('okr.review.needsWork', {
            title: worst.title,
            pct: String(Math.round(krProgress(worst) * 100)),
          }),
        );
      }
    }
    ...
    if (kids.length > 0) {
      lines.push(
        t('okr.review.cascaded', {
          list: kids.map((k) => `${k.title} ${objectiveProgress(objectives, k.id)}%`).join('; '),
        }),
      );
    }
  }
