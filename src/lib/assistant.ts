...
export function respondTo(
  ...
  if (/plan my day|build.*plan|schedule (my day|today)/.test(lower)) {
    ...
    const peakLine =
      peaks.length > 0 ? i18n.t('assist.r.peak', { hour: peaks[0].hour }) : '';
    return {
      text: i18n.t('assist.r.plan', { items: items.join(' → '), peak: peakLine }),
  ...
  if (/work on|next|should i|prioriti|focus/.test(lower)) {
    ...
    return {
      text: `${focus.headline} ${i18n.t('assist.r.next', { title: focus.task.title })}`,
      action: null,
    };
  }
  ...
  if (/matrix|eisenhower|quadrant|urgent/.test(lower)) {
    ...
    return {
      text: i18n.t('assist.r.matrix', {
        ...
        q4: c.q4,
        tail: i18n.t(
          c.q1 > 0 ? 'assist.r.matrix.q1' : c.q2 > 0 ? 'assist.r.matrix.q2' : 'assist.r.matrix.rest',
        ),
      }),
      action: null,
    };
