...
describe('history validation', () => {
  ...
  it('preserves a valid areaId alongside intention', () => {
    localStorage.setItem(
      'solanum:history',
      JSON.stringify([{ at: 1000, min: 25, intention: 'Ch. 2', areaId: 'area:thesis' }]),
    );
    expect(loadHistory()).toEqual([
  ...
  it('drops an invalid areaId without losing the session', () => {
    localStorage.setItem('solanum:history', JSON.stringify([{ at: 1000, min: 25, areaId: 7 }]));
  ...
  it('drops an invalid id without losing the session', () => {
    localStorage.setItem('solanum:history', JSON.stringify([{ id: 42, at: 1000, min: 25 }]));
    expect(loadHistory()).toEqual([{ at: 1000, min: 25 }]);
  });
});
...
describe('streak calculation (local days)', () => {
  ...
  const sessions = (offsets: number[]) => offsets.map((o) => ({ at: day(o), min: 25 }));
