...
import { SEEDED_AREA_CLOUD_IDS, ensureAreaCloudId, isUuid } from './areaIdentity';
import {
  createFocusArea,
  loadFocusAreas,
  markAreaDeleted,
...
describe('ensureAreaCloudId', () => {
  it('maps each seeded id to a valid, well-known cloud UUID', () => {
    for (const id of ['area:work', 'area:study', 'area:personal']) {
      ...
      expect(cloudId).toBe(SEEDED_AREA_CLOUD_IDS[id as keyof typeof SEEDED_AREA_CLOUD_IDS]);
    }
  });
