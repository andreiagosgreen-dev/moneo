...
import {
  extractCustomerPortalUrl,
  fetchCustomerPortalUrl,
} from '../../cloudflare/workers/portal';
...
describe('fetchCustomerPortalUrl (Worker → Lemon Squeezy API)', () => {
  ...
  it('returns null when the API is unreachable or has no portal URL', async () => {
    ...
    expect(await fetchCustomerPortalUrl({ apiKey: '' }, failing as unknown as typeof fetch)).toBeNull();
  });
});
...
describe('requestCustomerPortalUrl (browser → Worker)', () => {
  ...
  it('fails closed without a session, on HTTP errors or bad URLs', async () => {
    ...
    expect(await requestCustomerPortalUrl(async () => null, okFetch as unknown as typeof fetch)).toEqual({
      ok: false,
      url: null,
    });
