...
export type SoundType =
  'bell' | 'gong' | 'piano' | 'birds' | 'gentle' | 'rain' | 'ocean' | 'custom';
...
export interface Session {
  ...
  projectId?: string; // which Project this round belonged to (additive)
  taskId?: string; // which Task this round belonged to (Phase 2, additive)
}
...
export function loadSettings(): Settings {
  ...
  return {
    ...
    autoStart:
      typeof stored.autoStart === 'boolean' ? stored.autoStart : DEFAULT_SETTINGS.autoStart,
    ...
    .map((s) => {
      ...
      const intention =
        typeof s.intention === 'string'
          ...
          : undefined;
      const areaId = typeof s.areaId === 'string' && s.areaId.length > 0 ? s.areaId : undefined;
      const projectId =
        typeof s.projectId === 'string' && s.projectId.length > 0 ? s.projectId : undefined;
      const taskId = typeof s.taskId === 'string' && s.taskId.length > 0 ? s.taskId : undefined;
      const id = typeof s.id === 'string' && s.id.length > 0 ? s.id : undefined;
      return {
        ...(id ? { id } : {}),
        at: s.at,
        min: s.min,
...
export function durationFor(mode: Mode, s: Settings): number {
  return (mode === 'focus' ? s.focusMin : mode === 'short' ? s.shortMin : s.longMin) * 60;
}
...
export function minutesOnDay(history: Session[], day: Date): number {
  ...
  return history.filter((s) => dayKey(new Date(s.at)) === key).reduce((sum, s) => sum + s.min, 0);
}
...
/* ---------- completion chime (WebAudio via soundEngine) ---------- */
