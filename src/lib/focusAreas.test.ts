...
describe("area creation", () => {
  ...
  it("truncates names longer than 40 characters", () => {
    expect(createFocusArea([], "a".repeat(60))![0].name.length).toBe(
      AREA_NAME_MAX,
    );
  });
...
describe("storage robustness", () => {
  it("falls back to defaults on corrupt storage", () => {
    ...
    expect(loadFocusAreas().map((a) => a.name)).toEqual([
      "Work",
      "Study",
      "Personal",
    ]);
...
describe("round capture semantics", () => {
  ...
  it("the current round captures its area immutably", () => {
    ...
    expect(armRoundFocus("Write chapter 2", selected, areas).areaId).toBe(
      areas[1].id,
    );
  });
