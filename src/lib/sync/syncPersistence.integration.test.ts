...
function repos(
  ...
  return {
    ...
    pushSessions: async () => {
      if (opts.counters) opts.counters.sessions++;
      return true;
    },
    pushAreas: async () => {
      if (opts.counters) opts.counters.areas++;
      return true;
    },
    pushSettings: async () => {
      if (opts.counters) opts.counters.settings++;
      return true;
    },
  };
}
...
describe('production local sync persistence', () => {
  ...
  it('retries a failed canonical replacement once, then converges on two zero-op runs', async () => {
    ...
    expect(
      (await runSync({ userId: USER, consented: true, repos: cloud, local: createLocalSyncIO() }))
        .ok,
    ).toBe(false);
  ...
  it('does not fake remote adoption or R4A seeded-area mapping when history persistence fails', async () => {
    ...
    storage.seed(STORAGE_KEYS.focusAreas, [
      { id: 'area:work', cloudId: workCloud, name: 'Work', createdAt: 1 },
    ]);
    storage.failWrites.add(STORAGE_KEYS.history);
    const result = await runSync({
      ...
      consented: true,
      repos: repos({
        sessions: [{ id: 'remote', at: 2, min: 25, intention: null, areaId: workCloud }],
      }),
      local: createLocalSyncIO(),
    });
  ...
  it('does not apply a complete multi-page remote pull when the final local history write fails', async () => {
    ...
    storage.failWrites.add(STORAGE_KEYS.history);
    const all: RemoteSessionRow[] = [1, 2, 3].map((n) => ({
      id: `remote-${n}`,
      at: n + 10,
      min: 5,
      intention: null,
    ...
    const paged: SyncRepos = {
      ...repos(),
      pullSessions: () => pullPaged(async (from, to) => all.slice(from, to + 1), 1),
    };
  ...
  it('aborts before pull when a required local migration write fails', async () => {
    ...
    expect(result).toMatchObject({
      ok: false,
      stage: 'backfill',
      error: 'Could not prepare local storage.',
    });
    expect(loadSyncState().initialized).toBe(false);
  });
  ...
  it('propagates other sync-critical local write failures and never reports success when the marker cannot persist', async () => {
    ...
    install(storage);
    storage.failWrites.add(STORAGE_KEYS.focusAreas);
    const areaResult = await runSync({
      userId: USER,
      consented: true,
      repos: repos({
        areas: [{ id: 'a', name: 'Remote', createdAt: 1, updatedAt: 1, deletedAt: null }],
      }),
      local: createLocalSyncIO(),
    });
    ...
    const markerResult = await runSync({
      userId: USER,
      consented: true,
      repos: repos(),
      local: createLocalSyncIO(),
    ...
    expect(markerResult).toMatchObject({
      ok: false,
      stage: 'apply',
      error: 'Could not save sync state.',
    });
    expect(loadSyncState().initialized).toBe(false);
