...
import { SESSION_PULL_PAGE_SIZE, pullPaged } from '../cloud/sessionRepository';
...
describe('success / failure semantics', () => {
  ...
  it('a failed pull leaves sync state untouched and local intact', async () => {
    ...
    expect(store.history.map(({ at, min }) => ({ at, min }))).toEqual([{ at: 1000, min: 25 }]);
  });
...
describe('backfill + idempotence', () => {
  it('backfills ids onto legacy sessions and pushes them with those ids', async () => {
    ...
    expect(store.history.every((s) => typeof s.id === 'string' && s.id.length > 0)).toBe(true);
...
describe('merge behavior', () => {
  ...
  it('settings: local-newer pushes, remote-newer applies, equal no-ops', async () => {
    ...
    const resA = await runSync({
      userId: USER,
      consented: true,
      repos: reposFor(cloudA),
      local: localIO(localNewer),
    });
    ...
    const resB = await runSync({
      userId: USER,
      consented: true,
      repos: reposFor(cloudB),
      local: localIO(remoteNewer),
    });
    ...
    cloudC.settings = {
      focusMin: equal.settings.focusMin,
      shortMin: 5,
      longMin: 15,
      longEvery: 4,
      dailyGoal: 8,
...
describe('two-device convergence', () => {
  it('A1 + B1 converge exactly once on both devices with equal Growth', async () => {
    ...
    const r2 = await runSync({
      ...
      consented: true,
      repos: reposFor(cloud),
      local: localIO(deviceB),
    });
    expect(r2.adoptedSessions).toBe(1);
    deviceB.history = [...deviceB.history, { at: 2000, min: 50, intention: 'B one' }];
    const r3 = await runSync({
      userId: USER,
      consented: false,
      repos: reposFor(cloud),
    ...
    expect(getTotalFocusedMinutes(deviceA.history)).toBe(getTotalFocusedMinutes(deviceB.history));
  });
});
...
describe('privacy boundaries', () => {
  it('pushes ONLY sessions/areas/settings — never drafts or snapshots', async () => {
    ...
    for (const a of pushedAreas) {
      expect(new Set(Object.keys(a))).toEqual(new Set(['id', 'name', 'createdAt']));
    }
...
function pagedRepos(
  ...
  return {
    pullSessions: async () => {
      let pageCalls = 0;
      const sorted = [...cloud.sessions].sort((a, b) => a.at - b.at || a.id.localeCompare(b.id));
...
describe('R3 pagination × sync engine integration', () => {
  it('runSync consumes the COMPLETE multi-page remote history — no later-page row lost', async () => {
    ...
    expect(store.history.find((s) => s.id === 'pg-6')!.intention).toBe('Late page intention');
  ...
  it('local-only sessions survive and are pushed while a multi-page remote is adopted', async () => {
    ...
    expect(cloud.sessions.map((s) => s.id).sort()).toEqual(['local-only', 'r-1', 'r-2', 'r-3']);
  });
