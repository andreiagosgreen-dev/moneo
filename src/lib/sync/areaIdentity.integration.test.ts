...
import soundSettingsMigration from '../../../supabase/migrations/0003_settings_sound_notifications.sql?raw';
...
const migration = (name: string) =>
  name === '0001_core_schema.sql'
    ? coreMigration
    : name === '0002_user_scoped_area_identity.sql'
      ? scopedMigration
      : soundSettingsMigration;
let db: PGlite;
let user = A;
...
class Request implements PromiseLike<Result> {
  ...
  order(key: string, opts: { ascending: boolean }) {
    this.ordering.push(`${this.ident(key)} ${opts.ascending ? 'ASC' : 'DESC'}`);
    return this;
  }
  ...
  upsert(rows: Row | Row[], opts: { onConflict: string; ignoreDuplicates?: boolean }) {
    ...
    this.ignore = opts.ignoreDuplicates === true;
    return this;
  }
  update(patch: Row) {
    this.patch = patch;
  ...
  private async execute(): Promise<Result> {
    try {
      ...
      if (this.rows) {
        ...
        if (failAreaPush && this.table === 'focus_areas')
          throw Error('simulated area upload failure');
        for (const row of this.rows) {
          const cols = Object.keys(row);
          const keys = this.conflict!.split(',');
          const action = this.ignore
            ? 'DO NOTHING'
            : `DO UPDATE SET ${cols
                .filter((k) => !keys.includes(k))
                .map((k) => `${this.ident(k)}=EXCLUDED.${this.ident(k)}`)
                .join(',')}`;
          await db.query(
            `INSERT INTO public.${this.table} (${cols.map((k) => this.ident(k)).join(',')}) VALUES (${cols.map((_, i) => `$${i + 1}`).join(',')}) ON CONFLICT (${keys.map((k) => this.ident(k)).join(',')}) ${action}`,
            Object.values(row),
          );
        }
      ...
      const prefix = this.patch
        ? `UPDATE public.${this.table} SET ${Object.entries(this.patch)
            .map(([k, v]) => `${this.ident(k)}=${param(v)}`)
            .join(',')}`
        : `SELECT * FROM public.${this.table}`;
      const where = this.filters.length
        ? ` WHERE ${this.filters.map(([k, v]) => `${this.ident(k)}=${param(v)}`).join(' AND ')}`
        : '';
      const order = this.ordering.length ? ` ORDER BY ${this.ordering.join(',')}` : '';
      const range =
        this.start === undefined ? '' : ` LIMIT ${this.end! - this.start + 1} OFFSET ${this.start}`;
...
beforeAll(async () => {
  ...
  await db.exec(migration('0003_settings_sound_notifications.sql'));
  await db.exec(
...
beforeEach(async () => {
  await db.exec(
    `RESET ROLE; TRUNCATE auth.users CASCADE; INSERT INTO auth.users VALUES ('${A}'),('${B}');`,
  );
  writes.length = 0;
  errors.length = 0;
  ranges.length = 0;
...
describe('R4A cloud area identity', () => {
  it('Gate 0 exact reproduction: matching Work has no false insert or adoption', () => {
    ...
    expect(planAreaMerge([a], [remote(a)])).toMatchObject({
      pushInsertCount: 0,
      applyLocalCount: 0,
      noopCount: 1,
    });
  ...
  it.each([CUSTOM, 'area-custom-fallback'])(
    'pushes and converges custom local id %s without rewriting history',
    async (id) => {
      ...
      expect(
        (await db.query<{ area_id: string }>('SELECT area_id FROM focus_sessions')).rows[0].area_id,
      ).toBe(a.cloudId);
      await zeroOps();
  ...
  it('same-user devices converge all seeds; a second user owns independent rows', async () => {
    ...
    await zeroOps();
    const rows = (
      await db.query<{ user_id: string; id: string }>('SELECT user_id,id FROM focus_areas')
    ).rows;
  ...
  it('local-newer rename updates one cloud row and preserves both identities', async () => {
    ...
    expect((await db.query('SELECT name FROM focus_areas WHERE id=$1', [WORK])).rows).toEqual([
      { name: 'Job' },
    ]);
    await zeroOps();
  });
  it('remote-newer rename applies to the local id without duplicate adoption', async () => {
    await sync();
    await db.query(
      "UPDATE focus_areas SET name='Remote job',updated_at='2099-01-01' WHERE user_id=$1 AND id=$2",
      [A, WORK],
    );
    expect(await sync()).toMatchObject({ ok: true, pushedAreas: 0, appliedRemoteAreas: 1 });
    expect(loadFocusAreas()).toHaveLength(3);
    expect(loadFocusAreas()[0]).toMatchObject({
      id: 'area:work',
      cloudId: WORK,
      name: 'Remote job',
    });
  ...
  it('local soft delete keeps identity and the historical session reference', async () => {
    ...
    memory.set(
      K.focusAreas,
      JSON.stringify(markAreaDeleted(loadFocusAreas(), 'area:work', Date.now() + 1000)),
    );
    ...
    expect((await db.query('SELECT area_id FROM focus_sessions')).rows).toEqual([
      { area_id: WORK },
    ]);
    await zeroOps();
  });
  it('remote soft delete and later restoration converge without losing identity', async () => {
    await sync();
    await db.query(
      "UPDATE focus_areas SET deleted_at='2098-01-01',updated_at='2098-01-01' WHERE user_id=$1 AND id=$2",
      [A, WORK],
    );
    ...
    await zeroOps();
    await db.query(
      "UPDATE focus_areas SET deleted_at=NULL,updated_at='2099-01-01' WHERE user_id=$1 AND id=$2",
      [A, WORK],
    );
    expect(await sync()).toMatchObject({ ok: true, appliedRemoteAreas: 1 });
    expect(loadFocusAreas()[0]).toMatchObject({ id: 'area:work', cloudId: WORK });
  ...
  it('adopts a remote-only custom area and maps its session locally', async () => {
    ...
    await db.query(
      "INSERT INTO focus_sessions(id,user_id,completed_at,duration_min,area_id) VALUES($1,$2,'2026-01-01',25,$3)",
      [S, A, CUSTOM],
    );
    expect(await sync()).toMatchObject({ ok: true, appliedRemoteAreas: 1, adoptedSessions: 1 });
    expect(loadFocusAreas().find((a) => a.id === CUSTOM)).toMatchObject({
      cloudId: CUSTOM,
      name: 'Remote',
    });
  ...
  it('uploads seeded areas before their sessions through the production repositories', async () => {
    ...
    expect(errors).toEqual([]);
    expect((await db.query('SELECT user_id,area_id FROM focus_sessions')).rows).toEqual([
      { user_id: A, area_id: WORK },
    ]);
    await zeroOps();
    expect(memory.get(K.history)).toBe(before);
  });
  ...
  it('R3 actual paginated repository maps a later-page seeded session to its local id', async () => {
    await sync();
    await db.query(
      "INSERT INTO focus_sessions(id,user_id,completed_at,duration_min,area_id) VALUES($1,$2,'2026-01-01',10,NULL),($3,$2,'2026-01-02',25,$4)",
      [CUSTOM, A, S, WORK],
    );
    const repos = createSupabaseSyncRepos();
    repos.pullSessions = (id) => pullAllSessions(id, 1);
    expect(
      await runSync({ userId: A, consented: true, local: createLocalSyncIO(), repos }),
    ).toMatchObject({ ok: true, adoptedSessions: 2, appliedRemoteAreas: 0 });
  ...
  it('repository soft delete scopes the composite key to the requested owner', async () => {
    ...
    expect(
      (
        await db.query(
          'SELECT deleted_at=updated_at AS stamped FROM focus_areas WHERE user_id=$1 AND id=$2',
          [A, WORK],
        )
      ).rows,
    ).toEqual([{ stamped: true }]);
    const rows = (
      await db.query<{ user_id: string; deleted: boolean }>(
        'SELECT user_id,deleted_at IS NOT NULL AS deleted FROM focus_areas WHERE id=$1 ORDER BY user_id',
        [WORK],
      )
    ).rows;
    expect(rows).toEqual([
      { user_id: A, deleted: true },
      { user_id: B, deleted: false },
    ]);
  });
  ...
  it('additive migration preserves existing rows and rolls back invalid cross-owner history', async () => {
    ...
    try {
      ...
      await expect(
        legacy.exec(migration('0002_user_scoped_area_identity.sql')),
      ...
      ).toEqual([{ def: 'PRIMARY KEY (id)' }]);
      expect((await legacy.query('SELECT count(*)::int AS n FROM focus_sessions')).rows).toEqual([
        { n: 2 },
      ]);
