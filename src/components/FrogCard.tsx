...
export default function FrogCard({
  ...
  return (
    <section className="card px-6 py-6 sm:px-7" aria-label="Eat the frog">
      <header className="flex items-baseline justify-between gap-3">
        <div>
          <h2 className="font-display text-xl font-bold tracking-tight text-cream">Frog</h2>
          <p className="mt-1 font-mono text-[11px] uppercase tracking-[0.18em] text-faint">
            Eat the frog · hardest task first
          </p>
        </div>
        {isPro && stats.days > 0 && (
          <span className="font-mono text-[11px] text-sage" title="Frog streak (days)">
            🔥 {stats.streak}
          </span>
        )}
      </header>
      ...
      {!frog ? (
        <p className="mt-4 rounded-xl border border-dashed border-line/60 px-4 py-5 text-center text-[12px] leading-relaxed text-faint">
          No frogs left — every task is done.
          <br />
          Enjoy the clear pond.
        </p>
      ) : (
        <div className="mt-4 rounded-xl bg-ink/40 px-4 py-3.5 ring-1 ring-inset ring-line">
          ...
          <p className="mt-1 font-mono text-[11px] text-faint">
            ...
            {typeof frog.dueAt === 'number' &&
              ` · due ${new Date(frog.dueAt).toLocaleDateString([], { month: 'short', day: 'numeric' })}`}
            {eaten ? ' · eaten' : ' · uneaten'}
          </p>
          {!eaten && (
            <button
      ...
      {isPro && stats.days > 0 && (
        <div className="mt-3 flex items-center gap-4 font-mono text-[11px] text-faint">
          <span title="Frogs eaten">
            <span className="font-bold text-cream">{stats.done}</span>/{stats.days} eaten
          </span>
          <span title="Picked but never finished">
            <span className="font-bold text-tomato">{stats.skipped}</span> skipped
          </span>
      ...
      {!isPro && (
        <p className="mt-3 font-mono text-[11px] leading-relaxed text-faint">
          Pro tracks your frog streak and skip rate.
        </p>
      )}
    </section>
  );
