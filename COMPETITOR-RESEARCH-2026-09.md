...
This is Moneo's stated strategy ("combine instead of inventing a new idea") and the research confirms it's genuine white space — Motion/Sunsama/Akiflow/Reclaim are all calendar-and-tasks tools with zero project-management depth (no Gantt, no sprints, no OKRs). Moneo's Projects/Agile/OKR stack is a real differentiator *if* it stays reachable without breaking the calm-core experience (see §3.3). This directly supports doing the Projects redesign (item 5) — it's leaning into a gap, not chasing competitors.
